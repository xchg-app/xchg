...
attachments: att:9f2c…/shot.png att:77ab…/page.html   # проставляет --attach
---
# Заголовок
...
К сообщению можно приложить файлы: скриншот, html-страницу, лог. В ветку с письмами они не попадают.
Каждый файл становится коммитом без родителей, в котором лежит только он, под собственной ссылкой
`refs/xchg/att/<хеш>`, где хеш — это хеш блоба git этого файла; письмо только ссылается на него,
`att:<хеш>/<имя>`, в строке `attachments:`. Буквы в имени сохраняются на любом алфавите; пробелы и
прочие знаки, сколько бы их ни шло подряд, становятся одним `_`. Обычный pull — а значит, и каждый хук — берёт только
ветки, поэтому участник скачивает лишь те файлы, которые открывает:

```console
$ xchg send @api:bob layout-broken --attach shot.png --attach page.html <<< '# Шапка наезжает на меню'
$ xchg inbox
work     @api:bob     task  projects/api/bob/20260923-104500_alice-api_layout-broken.md alice/api Шапка наезжает на меню [+2 files]
...
Файлы выкладываются до того, как пишется письмо: если хаб какой-то не принял, не отправляется ничего.
Байты приходят такими же, какими ушли, включая переводы строк, и git по дороге сверяет их с хешем.
Одинаковый файл хранится один раз, кто бы его ни прислал снова. Предела размера клиент не ставит —
это дело сервиса, где живёт хаб, — и вложения не удаляет: ссылка живёт, пока её не уберёт человек
(`git push origin :refs/xchg/att/<хеш>`). Ссылка действительна только в своём хабе, поэтому
`xchg forward` переносит файлы вместе с письмом. Хостинг должен принимать ссылки вне веток и тегов;
распространённые принимают. На секреты вложения не проверяются так, как проверяется текст.
